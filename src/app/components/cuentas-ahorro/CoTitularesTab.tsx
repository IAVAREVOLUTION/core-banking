...
interface CoTitularesTabProps {
  mode: 'nuevo' | 'editar' | 'ver';
  accountId: number | 'new';
}

export function CoTitularesTab({ mode, accountId }: CoTitularesTabProps) {
  const getInitial = (): CoTitular[] => {
    const saved = loadFromSession<CoTitular[]>(accountId, 'cotitulares');
    if (saved) return saved;
    if (mode === 'nuevo') return [];
    return MOCK_COTITULARES[accountId as number] ? [...MOCK_COTITULARES[accountId as number]] : [];
  };

  const [cotitulares, setCotitulares] = useState<CoTitular[]>(getInitial);
  const [showModal, setShowModal] = useState(false);
  const [selectedIndex, setSelectedIndex] = useState<number | null>(null);
  const isReadOnly = mode === 'ver';

  useEffect(() => {
    if (mode !== 'ver') saveToSession(accountId, 'cotitulares', cotitulares);
  }, [cotitulares, accountId, mode]);

  const handleSelectCliente = (cliente: typeof CATALOGO_BUSQUEDA_CLIENTES[0]) => {
    const nombres = cliente.nombre.split(' ');
    const newC: CoTitular = {
      id: generateId(),
    ...
    setCotitulares(prev => [...prev, newC]);
    setShowModal(false);
    toast.success('Co-Titular agregado');
  };

  const handleEliminar = () => {
    if (selectedIndex === null) { toast.error('Seleccione un co-titular'); return; }
    setCotitulares(prev => prev.filter((_, i) => i !== selectedIndex));
    setSelectedIndex(null);
    toast.success('Co-Titular eliminado');
  };

  const update = (index: number, field: keyof CoTitular, value: any) => {
    setCotitulares(prev => prev.map((c, i) => i === index ? { ...c, [field]: value } : c));
  };
        ...
        <table className="w-full border-collapse min-w-[1000px]">
          <thead>
            <tr className="bg-gray-100 border-b border-gray-300">
              ...
              <th className="px-2 py-2 text-xs font-medium text-gray-700 text-center w-[70px]">Validado</th>
            </tr>
          </thead>
          <tbody>
            {cotitulares.length === 0 ? (
              <tr><td colSpan={9} className="px-3 py-6 text-center text-xs text-gray-400">{mode === 'nuevo' ? 'Agregue co-titulares con el boton Nuevo' : 'Sin co-titulares registrados'}</td></tr>
            ) : cotitulares.map((c, idx) => (
              <tr key={c.id} className={`border-b border-gray-300 cursor-pointer ${selectedIndex === idx ? 'bg-blue-50' : 'hover:bg-gray-50'}`} onClick={() => !isReadOnly && setSelectedIndex(idx)}>
                <td className="px-2 py-1.5 border-r border-gray-300"><div className="px-1 py-1 text-xs text-gray-700 bg-gray-50 border border-gray-300 rounded truncate">{c.claveCliente}</div></td>
                <td className="px-2 py-1.5 border-r border-gray-300"><div className="px-1 py-1 text-xs text-gray-700 bg-gray-50 border border-gray-300 rounded">{c.nombre}</div></td>
                <td className="px-2 py-1.5 border-r border-gray-300"><div className="px-1 py-1 text-xs text-gray-700 bg-gray-50 border border-gray-300 rounded">{c.apellidoPaterno}</div></td>
                <td className="px-2 py-1.5 border-r border-gray-300"><div className="px-1 py-1 text-xs text-gray-700 bg-gray-50 border border-gray-300 rounded">{c.apellidoMaterno}</div></td>
                <td className="px-2 py-1.5 border-r border-gray-300"><div className="px-1 py-1 text-xs text-gray-700 bg-gray-50 border border-gray-300 rounded">{fromISODate(c.fechaNacimiento)}</div></td>
                <td className="px-2 py-1.5 border-r border-gray-300">
                  <select value={c.parentesco} disabled={isReadOnly} onChange={e => { e.stopPropagation(); update(idx, 'parentesco', e.target.value); }} onClick={e => e.stopPropagation()} className="w-full px-1 py-1 text-xs border border-gray-300 rounded bg-white">
                    <option value="">Elige...</option>
                    {CATALOGO_PARENTESCO.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
                  </select>
                </td>
                <td className="px-2 py-1.5 border-r border-gray-300">
                  <input type="text" value={c.porcentaje} disabled={isReadOnly} onChange={e => { e.stopPropagation(); handlePercentChange(idx, e.target.value); }} onClick={e => e.stopPropagation()} className="w-full px-1 py-1 text-xs border border-gray-300 rounded bg-white text-right" />
                </td>
                <td className="px-2 py-1.5 border-r border-gray-300">
                  <input type="text" value={c.notas} disabled={isReadOnly} onChange={e => { e.stopPropagation(); update(idx, 'notas', e.target.value); }} onClick={e => e.stopPropagation()} className="w-full px-1 py-1 text-xs border border-gray-300 rounded bg-white" />
                </td>
                <td className="px-2 py-1.5 text-center">
                  <input type="checkbox" checked={c.validado} disabled={isReadOnly} onChange={e => { e.stopPropagation(); update(idx, 'validado', e.target.checked); }} onClick={e => e.stopPropagation()} className="w-4 h-4" />
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        {cotitulares.length < 3 && <div className="h-16 bg-white"></div>}
      </div>
